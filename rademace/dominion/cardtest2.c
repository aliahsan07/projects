/*
 * cardtest2.c
 * Tests for functionality of Great Hall.
 * Action and 1 Victory point
 * Cost: $3
 * +1 Card; +1 Action.
 *
 *
...
int main(int argc, char argv[]) {
	...
	initialNumBuys = 0;
	initialCoins = 7;
	greatHallCard = great_hall;
	card = (int) greatHallCard;
	choice1 = 0;
	choice2 = 0;
	choice3 = 0;
	...
	for (initialDeckCount = 0; initialDeckCount < 100; initialDeckCount += 10) {	
		for (initialHandCount = 0; initialHandCount < 20; initialHandCount += 2) {
			for (initialDiscardCount = 1; initialDiscardCount < 100; initialDiscardCount += 10) {
				...
				assert(0 == ret_val);
				assert(initialNumActions + 1 == state->numActions);
	...
	printf("Great Hall card passed all tests\n");
