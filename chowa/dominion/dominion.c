...
int play_mine(int player, struct gameState *state, int handPos, int choice1, int choice2)
{
      ...
      if (state->hand[player][choice1] < copper || state->hand[player][choice1] > gold)
	{
	  return -1;
		...
		//this will be a bug/severe laziness i'll need to deal with later
...
{	
      ...
      for (i = 0; i < state->handCount[player]; i++)
	{
	  if (state->hand[player][i] == j)
	    {
	      discardCard(i, player, state, 0);			
	      i = state->handCount[player]+1; //break out of loop
	    }
	}
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
	i = total_card(currentPlayer, state)-state->handCount[currentPlayer];
    while(drawntreasure<10 && z < i){
    ...
    case mine:
      play_mine(currentPlayer, state, handPos,choice1,choice2);
			
    case remodel:
	  play_remodel(currentPlayer, state, handPos, choice1, choice2);
