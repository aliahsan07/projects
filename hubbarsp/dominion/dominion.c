...
int endTurn(struct gameState *state) {
  ...
  for (i = 0; i < state->handCount[currentPlayer]; i++) {
    state->discard[currentPlayer][state->discardCount[currentPlayer]++] = state
...
// Tribute card effect.
static int tributeEffect(struct gameState *state) {
  int i;
  int currentPlayer = whoseTurn(state);
...
// Treasure map card effect.
static int treasureMapEffect(struct gameState *state, int handPos) {
...
// Feast card effect.
static int feastEffect(struct gameState *state, int choice) {
...
// Baron card effect.
static int baronEffect(struct gameState *state, int choice) {
  int currentPlayer = whoseTurn(state);
...
int cardEffect(int card, int choice1, int choice2, int choice3,
  ...
  switch (card) {
    case adventurer: return adventurerEffect(state);
    ...
    case feast: return feastEffect(state, choice1);
    ...
    case baron: return baronEffect(state, choice1);
    ...
    case tribute: return tributeEffect(state);
    ...
    case sea_hag:
      for (i = 0; i < state->numPlayers; i++) {
        if (i != currentPlayer) {
          state->discard[i][state->discardCount[i]] = state->deck[i][state
              ->deckCount[i]--];
          state->deckCount[i]--;
    ...
    case treasure_map: return treasureMapEffect(state, handPos);
  }
...
int discardCard(int handPos, int currentPlayer, struct gameState *state,
  ...
  if (handPos == (state->handCount[currentPlayer] - 1)) //last card in hand array is played
    ...
    state->hand[currentPlayer][handPos] = state->hand[currentPlayer][(state
