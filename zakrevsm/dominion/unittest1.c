...
int main(int argc, char* argv[]){
	...
	//Player 1
	for(i = 0; i < 10; i++){
		gainCard(curse, p,0 , 0);
	}
	gainCard(province, p, 0, 0);
	
	printf("Player 1: Cards in deck = %d\n", p->deckCount[0]);
	
	//player2
	for(i = 0; i< 7; i++){
		gainCard(province, p, 0, 1);
	}
	...
	for(i = 0; i< 4; i++){
		gainCard(gardens, p,0 , 0);
	}
	
	for(i = 0; i< 6; i++){
		gainCard(smithy, p,0, 0);
	}
	...
	for(i = 0; i< 10; i++){
		gainCard(silver, p, 0, 0);
	}
	for(i = 0; i< 10; i++){
		gainCard(copper, p, 0, 0);
	}
	
	//player2
	for(i = 0; i< 10; i++){
		gainCard(estate, p, 0, 1);
	}
	
	for(i = 0; i< 9; i++){
		gainCard(copper, p, 0, 1);
	}
	
	gainCard(gardens, p, 0, 1);
