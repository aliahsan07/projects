...
int r_village(int currentPlayer, struct gameState *state, int handPos)
{
  //+1 Card
...
int r_great_hall(int currentPlayer, struct gameState *state, int handPos)
{
  //+1 Card
  drawCard(currentPlayer, state);
  //+1 Actions
...
int r_embargo(int currentPlayer, struct gameState *state, int handPos, int choice1)
{
  //+2 Coins
  state->coins = state->coins + 2;
  //see if selected pile is in play
  if (state->supplyCount[choice1] == -1) return -1;
...
int r_outpost(int currentPlayer, struct gameState *state, int handPos)
{
    //set outpost flag
    state->outpostPlayed++;
    //discard card
...
int initializeGame(int numPlayers, int kingdomCards[10], int randomSeed, struct gameState *state) {
  ...
  if (numPlayers > MAX_PLAYERS || numPlayers < 2) return -1;
  ...
  for (i = 0; i < 10; i++)
  {
    for (j = 0; j < 10; j++)
    {
     if (j != i && kingdomCards[j] == kingdomCards[i]) return -1;
    }
  }
...
int getWinners(int players[MAX_PLAYERS], struct gameState *state) {
  ...
  {   
    ...
    if (i >= state->numPlayers) players[i] = -9999;
    else players[i] = scoreFor (i, state);
  }
  ...
  for (i = 0; i < MAX_PLAYERS; i++)
  {
    if ( players[i] == highScore && i > currentPlayer ) players[i]++;
  }
  ...
  for (i = 0; i < MAX_PLAYERS; i++)
  {
    if ( players[i] == highScore ) players[i] = 1;
...
{ int count;
  ...
  if (state->deckCount[player] <= 0)   //Deck is empty
  {
    ...
    if (DEBUG) printf("Deck count now: %d\n", state->deckCount[player]);
    ...
    if (DEBUG) printf("Current hand count: %d\n", count);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
  {
    ...
    case smithy:
      return r_smithy(currentPlayer, state, handPos);
    
    case village:
      return r_village(currentPlayer, state, handPos);
    ...
    case great_hall:
      return r_great_hall(currentPlayer, state, handPos);
    ...
    case minion:
      ...
      {
        //discard hand
        while(numHandCards(state) > 0) discardCard(handPos, currentPlayer, state, 0);
        ...
        for (i = 0; i < state->numPlayers; i++)
        {
          if (i != currentPlayer)
          {
            ...
            {
              //discard hand
              while( state->handCount[i] > 0 ) discardCard(handPos, i, state, 0);
            }
              
            //draw 4
            for (j = 0; j < 4; j++) drawCard(i, state);
          }
        } 
      }
    ...
    case steward:
      ...
      else if (choice1 == 2) state->coins = state->coins + 2;
    ...
    case embargo: 
      return r_embargo(currentPlayer, state, handPos, choice2);
    
    case outpost:
      return r_outpost(nextPlayer, state, handPos);
    ...
    case treasure_map:
      ...
      if (index > -1)
      {
        ...
        //gain 4 Gold cards
        for (i = 0; i < 4; i++) gainCard(gold, state, 1, currentPlayer);
