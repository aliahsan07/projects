...
int main(int argc, char** argv) {
	...
	for(i=0; i<MAX_TESTS; i++){
		...
		if(massert (G.handCount[0] == handCount + 1, "handCount incorrect")){
			printf ("cards (after, before):  ");
			printf ("%d, %d\n", G.handCount[0], handCount);
			testfailed++;
		}
		if(massert (G.numActions >= numActions + 1, "numActions incorrect")){
			printf ("numActions(after, before):  ");
