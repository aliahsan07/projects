...
{	
  ...
  //updateCoins(state->whoseTurn, state, coin_bonus);  THIS LINE IS INCORRECT, SO IT HAS BEEN COMMENTED OUT
...
int card_Feast(int i, int x, int currentPlayer, int temphand [], struct gameState *state, int choice1, int z, int handPos){
	...
	else
  {
    ...
    discardCard(handPos, currentPlayer, state, 1);//Trash Feast
...
int card_Ambassador(int i, int j, int choice1, int choice2, int handPos, struct gameState *state, int currentPlayer){
  j = 0;		//used to check if player has enough cards to discard
	...
	if (choice2 > 2 || choice2 < 0 || choice1 < 0)
	{
		return -1;
	}
...
int card_Tribute(int i, struct gameState *state, int currentPlayer, int tributeRevealedCards [], int nextPlayer,int handPos){
...
int card_Baron(int i, int choice1, struct gameState *state, int currentPlayer, int handPos){
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
	...
	//Refactored card #2
	case feast:
		return card_Feast(i, x,currentPlayer, temphand, state, choice1, z, handPos);
	...
	//Refactored card #4
	case tribute:
		return card_Tribute(i, state, currentPlayer, tributeRevealedCards, nextPlayer,handPos);

	//Refactored card #5
	case baron:
		return card_Baron(i, choice1, state, currentPlayer, handPos);
