...
int doAdventurer(int currentPlayer, struct gameState *state){
	int drawntreasure = 0;
	int cardDrawn;
	...
	while(drawntreasure<2){
		...
		if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold)
		  drawntreasure++;
			...
			state->discard[currentPlayer][(state->discardCount[currentPlayer])+1] = cardDrawn;
...
int doCouncil_room(int handPos, int currentPlayer, struct gameState *state){
	int i;
	for (i = 0; i < 4; i++)
	{
	  drawCard(currentPlayer, state);
...
int doVillage(int handPos, int currentPlayer, struct gameState *state){
 //+1 Card
      drawCard(currentPlayer, state);
...
int doGreat_hall(int handPos, int currentPlayer, struct gameState *state){
	 //+1 Card
      drawCard(currentPlayer, state);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
	  doAdventurer(currentPlayer, state);
      return 0;
			
    case council_room:
      //+4 Cards
	  doCouncil_room(handPos, currentPlayer, state);
    ...
    case smithy:
     doSmithy(handPos, currentPlayer, state);
      return 0;
		
    case village:
     doVillage(handPos, currentPlayer, state);
      return 0;
    ...
    case great_hall:
	  doGreat_hall(handPos, currentPlayer, state);
      return 0;
